...
**Where to write it.** If you cloned the LOOM monorepo, write into `content/modules/<kind>/`. If you installed LOOM as a dependency (no writable `content/`), write into a **module pack** instead — the same path inside your pack (`<pack>/modules/<kind>/<name>.ts`), registered with `pack:add` (see the **library-use** skill). The `defineModule` contract, the import rule (`@loom/runtime` + `three` only), and the test-case requirement are identical either way — that sameness is exactly what makes a pack portable.
...
- **Never sample a raw `texture()` inside a TSL `Fn()` closure.** A texture-sample node built inside an `Fn` function-scope isn't collected into the material's sampler bindings (three's node backend), so the sampler reads unbound and the shader silently renders **BLACK** (build ok, `instanceError` null — nothing tells you). `uniform()` nodes cross the `Fn` boundary fine; texture samples do not. So sample textures at the **top level** of `build()` and pass the value in. (`ctx.palette.ramp()` is **safe** inside an `Fn` — it's built from the stop *uniforms* via an in-shader `mix`, not a texture sample; `mandelbulb` uses it inside its raymarch loop.) A golden-pattern scan rejects a raw `texture(` inside an `Fn(`.
